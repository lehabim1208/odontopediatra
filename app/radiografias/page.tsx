...
export default function RadiografiaPage() {
  ...
  let patientId: string | null = searchParams.get("patient")
  const [patient, setPatient] = useState<any>(null)

  useEffect(() => {
    let id: string | null = patientId
    if (!id && typeof window !== 'undefined') {
      id = sessionStorage.getItem('selectedPatientId')
    }
    if (id) {
      const patients = storage.getItem("patients") || []
      const foundPatient = patients.find((p: any) => p.id.toString() === id)
      if (foundPatient) {
        setPatient(foundPatient)
        // Limpiar el ID de sessionStorage después de usarlo
