...
export function Sidebar({ onExpandChange }: { onExpandChange?: (expanded: boolean) => void }) {
  ...
  const { setIsLoggedIn, userRole, isDarkMode, toggleDarkMode, hasPermission, permissionsLoaded } = useAuth()
  ...
  const filteredMenuItems = permissionsLoaded ? menuItems.filter((item) => item.permission) : []

  return (
    <>
      {showLogoutLoader && <PageTransition />}
      {/* Mostrar loader mientras no cargan los permisos */}
      {!permissionsLoaded && <PageTransition />}
