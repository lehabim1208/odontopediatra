...
interface RadiographUploaderProps {
  title?: string
  onFileAdded?: (file: FileItem & { file?: File }) => void
...
export function RadiographUploader({
  ...
  const handleDownloadFile = async (file: FileItem) => {
    ...
    try {
      const response = await fetch(getApiFileUrl(file.url))
  ...
  // Al mostrar archivos guardados, reescribe la URL para usar el endpoint seguro
  const getApiFileUrl = (url: string) => {
    if (!url) return url
    // Si ya es una URL absoluta o de API, no la cambies
    if (url.startsWith("/api/archivos/")) return url
    // Extrae el nombre del archivo de la URL original
    const match = url.match(/\/archivos\/(.+)$/)
    if (match) return `/api/archivos/${match[1]}`
    return url
  }
    ...
    <>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              >
                                {downloadingId === file.id ? (
                                  <>
                                    <Download className="h-4 w-4 mr-2 animate-spin" /> Descargando...
                                  </>
                                ) : (
                                  <>
            ...
            {selectedFile && previewFile ? (
              <div className="space-y-4">
                <FilePreview
                  ...
                  fileUrl={getApiFileUrl(previewFile.url)}
                  maxHeight={400}
                  zoomLevel={zoomLevel}
                />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-end gap-2 mt-4">
                    ...
                    <Button size="sm" onClick={() => handleDownloadFile(selectedFile)} disabled={downloadingId === selectedFile.id}>
                      {downloadingId === selectedFile.id ? (
                        <>
                          <Download className="mr-2 h-4 w-4 animate-spin" /> Descargando...
                        </>
                      ) : (
                        <>
      ...
      <Dialog open={showFullPreviewModal} onOpenChange={setShowFullPreviewModal}>
        <DialogContent className="sm:max-w-[90vw] sm:max-h-[90vh] p-0">
          <DialogTitle className="sr-only">Vista previa de documento</DialogTitle>
