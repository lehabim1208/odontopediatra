...
const TutorForm: React.FC<TutorFormProps> = ({ paciente: initialPaciente, onSuccess, onTutorCreated, onCancel, tutoresCount = 0 }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!relacion) {
      setError('Selecciona la relación con el paciente');
      return;
    }
    setLoading(true);
    setError('');
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  required
                  className="w-full border rounded p-2 mt-1 bg-background text-foreground border-border focus:outline-none focus:ring-2 focus:ring-primary"
                >
